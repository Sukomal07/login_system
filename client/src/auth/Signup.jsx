...
function SignUp() {
    ...
    async function createAccount(event) {
        ...
        try {
            const response = axiosInstance.post("/user/signup", formData);
            toast.dismiss()
            ...
            <div className='flex flex-col gap-3 justify-center items-center h-[91vh]'>
                <form onSubmit={createAccount} className='lg:w-[450px] w-[90%] md:w-1/2 h-fit p-7 flex flex-col gap-5 justify-between rounded-md bg-white text-black shadow-md'>
                    ...
                    <div className='flex items-center w-full gap-4 border-2 border-yellow-500 px-4 rounded-lg h-14 bg-slate-900'>
                        <label htmlFor="userName" className='text-xl hidden lg:block md:block text-yellow-500'><BsPerson /></label>
                        <input type="text" name='userName' id='userName' placeholder="Enter username" className="py-2 border-0 outline-0 text-xl text-white bg-transparent w-full " onChange={handleUserInput} />
                    </div>
                    <div className='flex items-center w-full gap-4 border-2 border-yellow-500 px-4 rounded-lg h-14 bg-slate-900'>
                        <label htmlFor="fullName" className='text-xl hidden lg:block md:block text-yellow-500'><BsPerson /></label>
                        <input type="text" name='fullName' id='fullName' placeholder="Enter fullname" className="py-2 border-0 outline-0 text-xl text-white bg-transparent w-full " onChange={handleUserInput} />
                    </div>
                    <div className='flex items-center w-full gap-4 border-2 border-yellow-500 px-4 rounded-lg h-14 bg-slate-900'>
